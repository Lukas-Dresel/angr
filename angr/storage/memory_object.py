...
class SimMemoryObject(object):
    ...
    def bytes_at(self, addr, length, allow_bytes=False):
        if addr == self.base and length == self.length:
            return claripy.BVV(self.object) if not allow_bytes and self.is_bytes else self.object

        if self.is_bytes:
            start = addr - self.base
            end = start + length
            o = self.object[start:end]
            return o if allow_bytes else claripy.BVV(o)
        obj_size = self.size()
