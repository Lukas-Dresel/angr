...
class SimLyingRegArg(SimRegArg):
    ...
    def get_value(self, state, size=None, endness=None, **kwargs):  # pylint:disable=arguments-differ
        ...
        if endness and endness != state.arch.register_endness:
            val = val.reversed
        if size == 4:
            val = claripy.fpToFP(claripy.fp.RM_NearestTiesEven, val.raw_to_fp(), claripy.FSORT_FLOAT)
        return val
    ...
    def set_value(self, state, val, size=None, endness=None, **kwargs):  # pylint:disable=arguments-differ
        if size == 4:
            if state.arch.register_endness == 'IEnd_LE' and endness == 'IEnd_BE':
                ...
                val = claripy.fpToFP(claripy.fp.RM_NearestTiesEven, val.reversed.raw_to_fp(), claripy.FSORT_DOUBLE).reversed
            else:
                val = claripy.fpToFP(claripy.fp.RM_NearestTiesEven, val.raw_to_fp(), claripy.FSORT_DOUBLE)
        if endness and endness != state.arch.register_endness:
            val = val.reversed
